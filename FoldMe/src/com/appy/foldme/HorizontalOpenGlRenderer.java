...
public class HorizontalOpenGlRenderer implements Renderer {
    ...
    private Page[] Folds = new Page[HorizontalFoldingMenu.H_TOTAL];
    private ExternalPage ExtPage;

    private Context renderContext;

    private float c = 1 - ((float) 1/ HorizontalFoldingMenu.H_TOTAL);

    private float lastangle = 90 / (float) Math.pow(c,((HorizontalFoldingMenu.H_TOTAL-1)/2));

    // Declare as volatile because we are updating it from another thread
    public volatile float mAngle;

	/***** OpenGlRenderer Constructor *****/
	public HorizontalOpenGlRenderer(Context context) {
	...
	public void onDrawFrame(GL10 gl) {
        ...
        drawLight();
            ...
            for (int i = 0; i < Folds.length; i++){
                ...
                // Page Transformation
                if (i % 2 == 0){
                    drawEvenPage(Folds[i],i,angle,trans,HorizontalFoldingMenu.H_DIRECTION);
                } else {
                    drawOddPage(Folds[i],i,angle,trans,HorizontalFoldingMenu.H_DIRECTION);
                }

                if (i == Folds.length-1) {
                    float exttrans;
                    if (i % 2 == 0){
                        exttrans = 2-(2-(HorizontalFoldingMenu.H_TOTAL)*Folds[i].getwidth())-trans; // This is currently not supported but needs some fixing (OR ELSE WE NEED TO THROW SOME EXCETIONS)
                    } else {
                        exttrans = 2-(2-HorizontalFoldingMenu.H_TOTAL*Folds[i].getwidth())-trans;
                    }
                    drawExternalPage(exttrans,HorizontalFoldingMenu.H_DIRECTION);
                }
            }
	}
...
//	        float ratio = (float) height / width;
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        for (int i = 0; i < HorizontalFoldingMenu.H_TOTAL;i++){
            Folds[i] = new Page(renderContext,i,HorizontalFoldingMenu.H_TOTAL,HorizontalFoldingMenu.H_DIRECTION);
        }

        ExtPage = new ExternalPage(renderContext);

        setTexture(HorizontalFoldingMenu.H_bmp, GLES20.GL_TEXTURE0, 0);
        setTexture(HorizontalFoldingMenu.H_extbmp,GLES20.GL_TEXTURE1,1);

//        Log.d(HorizontalFoldingMenu.DEBUG_TAG, "ONSURFACECREATED");
