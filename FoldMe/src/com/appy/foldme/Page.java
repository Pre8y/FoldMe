...
//    }
    ...
    public Page(Context context, int pos, int total, int direction) {

        Resources res = context.getResources();
        ratio = (float) res.getInteger(R.integer.GapViewWeight)/(res.getInteger(R.integer.CapturedViewWeight)+res.getInteger(R.integer.GapViewWeight));

        setwidth((float) (2 * ratio) / total);
        mContext = context;

        // vertexes coordinates in counterclockwise order:
        float pageCoordsEven[] = {
                0.0f,  1.0f, 0.0f,   // 0. top left
                0.0f, -1.0f, 0.0f,   // 1. bottom left
                width, -1.0f, 0.0f,  // 2. bottom right
                width,  1.0f, 0,0f   // 3. top right
                };
        float pageCoordsOdd[] = {
                -width,  1.0f, 0.0f, // 0. top left
                -width, -1.0f, 0.0f, // 1. bottom left
                0.0f, -1.0f, 0.0f,   // 2. bottom right
                0.0f,  1.0f, 0,0f    // 3. top right
                 };
        float pageCoords[] = new float[12];
        ...
        float TextureCoordinateData[] = new float[8];
        float TextureCoordinateDataRL[] =
                {
                        pos * (width/2)/ratio, 0.0f,
                        pos * (width/2)/ratio, 1.0f,
                        (pos+1) * (width/2)/ratio, 1.0f,
                        (pos+1) * (width/2)/ratio, 0.0f
                };
        float TextureCoordinateDataLR[] =
                {
                        (1 - ((pos+1) * (width/2))/ratio), 0.0f,
                        (1 - ((pos+1) * (width/2))/ratio), 1.0f,
                        (1 - (pos * (width/2))/ratio), 1.0f,
                        (1 - (pos * (width/2))/ratio), 0.0f
                };


        if (direction == 1) {
            if (pos % 2==0) pageCoords = pageCoordsEven;
            else pageCoords = pageCoordsOdd;
            TextureCoordinateData = TextureCoordinateDataRL;

        }
        else if (direction == -1) {
            if (pos % 2 ==0) pageCoords = pageCoordsOdd;
            else pageCoords = pageCoordsEven;
            TextureCoordinateData = TextureCoordinateDataLR;
        }
